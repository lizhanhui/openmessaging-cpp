...
PullConsumerImpl::PullConsumerImpl(jobject proxy) : ServiceLifecycleImpl(proxy) {
    ...
    midAttachQueue = current.getMethodId(classPullConsumer, "attachQueue",
                                         buildSignature(Types::PullConsumer_, 1, Types::String_));
    midAttachQueue2 = current.getMethodId(classPullConsumer, "attachQueue",
                                          buildSignature(Types::PullConsumer_, 2, Types::String_, Types::KeyValue_));
    midDetachQueue = current.getMethodId(classPullConsumer, "detachQueue",
                                         buildSignature(Types::PullConsumer_, 1, Types::String_));
    midReceive = current.getMethodId(classPullConsumer, "receive", buildSignature(Types::Message_, 0));
    midReceive2 = current.getMethodId(classPullConsumer, "receive",
                                      buildSignature(Types::Message_, 1, Types::KeyValue_));
    midAck = current.getMethodId(classPullConsumer, "ack", buildSignature(Types::void_, 1, Types::String_));
    midAck2 = current.getMethodId(classPullConsumer, "ack",
                                  buildSignature(Types::void_, 2, Types::String_, Types::KeyValue_));
}
...
KeyValuePtr PullConsumerImpl::attributes() {
    ...
    KeyValueImplPtr ptr(new KeyValueImpl(current.newGlobalRef(kv)));
...
MessagePtr PullConsumerImpl::receive(const KeyValuePtr &props) {
    ...
    if (jMessageLocal) {
        ...
        MessagePtr messagePtr(new ByteMessageImpl(jMessage));
        return messagePtr;
    } else {
